...
{
    public partial class SituationLocataireView : UserControl, ISituationLocataireView
    {
        ...
        private void SituationLocataireView_Load(object sender, EventArgs e)
        {
            ...
            this.olv_report.Scroll += this.OlvReportOnScroll;
        }

        private void OlvReportOnScroll(object sender, ScrollEventArgs scrollEventArgs)
        {
            OLVListItem value = this.olv_report.GetItem(this.olv_report.TopItemIndex);

            if (value.RowObject is LigneSituationRo lsr && lsr._Contrat != this.lb_titre.Text)
                this.lb_titre.Text = lsr._Contrat;
        }

        private void OlvReportOnCellOver(object sender, CellOverEventArgs args)
        {
            if (args.Model is LigneSituationRo lsr &&
                (!string.IsNullOrEmpty(lsr._Contrat) && lsr._Contrat != this.lb_titre.Text))
                this.lb_titre.Text = lsr._Contrat;
        }

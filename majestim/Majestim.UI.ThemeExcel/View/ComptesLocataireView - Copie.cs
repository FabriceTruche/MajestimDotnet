...
using log4net;
using Majestim.BL.Compta;
using Majestim.BO.Compta;
using Majestim.BO.Contrat;
...
//        private IEnumerable<Contrat> _allContrats;
        ...
        private Func<Compte, bool> ComptesDefaultFunc => compte => compte.Numero.StartsWith("41");
        private Func<Compte, bool> ComptesTous => compte => true;
        private Func<Compte, bool> ComptesAucun => compte => false;
        ...
        {
            // inti lv
            Generator.GenerateColumns(this.olv_report, typeof(LigneComptesLocataireReport));
        ...
        private void OlvReport_OnFormatCell(object sender, FormatCellEventArgs arg)
        {
            if (arg.Column.AspectName == "Periode" &&
                arg.Model is LigneComptesLocataireReport lsr &&
                (lsr.LigneComptesLocataireReportType != LigneComptesLocataireReportType.EmptyRow))
            {
            ...
            {
                arg.SubItem.Font = new Font(arg.SubItem.Font.FontFamily, arg.SubItem.Font.Size - 2);
            }
        }

        private void OlvReport_OnFormatRow(object sender, FormatRowEventArgs args)
        {
            if (args.Model is LigneComptesLocataireReport lsr)
            {
                switch (lsr.LigneComptesLocataireReportType)
                {
                    case LigneComptesLocataireReportType.Summary:
                        this.SetRowFormat(
                            args.Item,
                            DebitCreditHelper.SignOf(lsr.Debit, lsr.Credit), this._colors[normalColor]);
                        break;

                    case LigneComptesLocataireReportType.EmptyRow:
                        break;
                }
            }
        }
        ...
        private void btn_Update_Click(object sender, EventArgs e)
        {
            ...
            this.UpdateReportClicked?.Invoke(
                this.combo_contrat.SelectedItem as Contrat,
        ...
        public void ProvideContrats(IEnumerable<Contrat> allContrats)
        {
            //this._allContrats = allContrats;
            //this.UpdateContratsList(false);
        ...
        public void ProvideComptes(IEnumerable<Compte> comptes)
        {
        ...
        private void UpdateSelectionComptes(Func<Compte, bool> func)
        {
            for (int i = 0; i < this.lb_compte.Items.Count; i++)
            {
                if (this.lb_compte.Items[i] is Compte compte)
                {
        ...
        public void ProvideCodeOperations(IEnumerable<CodeOperation> operations)
        {
            this.lb_operation.Items.Clear();
            foreach (CodeOperation operation in operations)
        ...
        public void ProvideReport(IEnumerable<LigneComptesLocataireReport> rows)
        {
        ...
        {
            return
                this.lb_operation.Items.Cast<CodeOperation>().Where((t, i) => this.lb_operation.GetSelected(i)).Select(t => t.Code).ToList();
        }
        ...
        {
            return
                this.lb_compte.Items.Cast<Compte>().Where((t, i) => this.lb_compte.GetSelected(i)).Select(t => t.Numero).ToList();
        }
